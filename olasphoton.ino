...
void setup() {
  ...
  Spark.subscribe("olamicek", myHandler);
...
void loop() {
  ...
  if (analogRead(photoresistor)>beamThreshold) {
    if (beamBroken==true) {
        Spark.publish("cegandodge","intact");
      ...
      if (beamBroken==false) {
        // Same deal as before...
        Spark.publish("cegandodge","broken");
        beamBroken=true;
         Particle.publish("olamicek", "asdfgh1111");
    delay(2000);
      }
  }
}
...
void myHandler(const char *event, const char *data)
{
  ...
  */
  ...
  if (strcmp(data,"intact")==0) {
    ...
    digitalWrite(boardLed,LOW);
    Particle.publish("cegandodge", "intact");
    delay(2000);
  }
